...
datapath = Path(__file__).parent.parent / "testdata"
...
MINIMAP_SFD = sfdmap.SFDMap(
    "testdata", north="SFD_dust_4096_ngp_cutout.fits", scaling=1.0
)
...
def test_versus_ned():
    ...
    NED_RESULTS = [
        {"ra": 204.17470, "dec": -29.960283, "landoltv": 0.212},
...
@pytest.mark.skipif(
    "SFD_DIR" not in os.environ or not HAVE_FITS_READER,
...
def test_boundaries():
    ...
    for interp in (False, True):
        for l in (0.0, np.pi / 2, np.pi, 3.0 * np.pi / 2.0):
            m.ebv(l, 0.0, frame="galactic", unit="radian", interpolate=interp)
...
def test_convenience_func():
    ...
    ebv2 = sfdmap.ebv(
        204.0, -30.0, mapdir="testdata", north="SFD_dust_4096_ngp_cutout.fits"
    )
...
def test_interpolate_false():
    ...
    ebv = MINIMAP_SFD.ebv(206.61462, -30.441592, frame="fk5j2000", interpolate=False)
